...
// edit line 38	
	 ...
	 */
	constructor() {
		this.name = "DAGsearchWui";
		this.version = 1.1;
		this.author = "Peter Spry";
		this.pcfg = getPluginConfig(this.name);
		this.tabName = (this.pcfg.tabName) ? this.pcfg.tabName : "Search";
		this.tabContentFile = "application/plugins/DAGsearchWui/tab-content.html";
		this.response = "";
	...
	formatDT(dt) {
		...
		var month = app.plugins.DAGsearchWui.zeroPad(dt.getMonth()+1, 2);
		var date = app.plugins.DAGsearchWui.zeroPad(dt.getDate(), 2);
	...
	formatDate(date) {
		...
		var day = app.plugins.DAGsearchWui.zeroPad(date.getDate(),2);
		var monthIndex = app.plugins.DAGsearchWui.zeroPad(date.getMonth()+1,2);
	 ...
	 */
	applyFilter() {
		var cqlFilter = app.plugins.DAGsearchWui.getCqlFilter();
		...
		setCqlFilter(filterLayer, app.plugins.DAGsearchWui.name, cqlFilter);		
	}		
	...
	addFilterControls() {
		...
		app.plugins.DAGsearchWui.filter.fields.forEach(function(field) {
			...
			switch (field.type) {	
				case "choice":
					...
					control.find("select").on( 'change', function(){
						...
						app.plugins.DAGsearchWui.applyFilter();						
					});
					break;
				...
				case "dateRange":
					...
					control.find(".input-group-text").html(field.prettyName);
					control.attr("fieldName", field.name);
					control.find(".dagSearch-amount1").val(app.plugins.DAGsearchWui.formatDate(field.from)); 
					control.find(".dagSearch-amount2").val(app.plugins.DAGsearchWui.formatDate(field.to));			
					control.find(".dagSearch-amount1").attr("min",app.plugins.DAGsearchWui.formatDate(field.from)); 
					control.find(".dagSearch-amount1").attr("max",app.plugins.DAGsearchWui.formatDate(field.to)); 
					control.find(".dagSearch-amount2").attr("min",app.plugins.DAGsearchWui.formatDate(field.from));	
					control.find(".dagSearch-amount2").attr("max",app.plugins.DAGsearchWui.formatDate(field.to));			
					control.find(".dagSearch-slider-range").slider({
						...
						stop: function( event, ui ) {
							...
							control.find(".dagSearch-amount1").val(app.plugins.DAGsearchWui.formatDate(stopFrom)); 
							control.find(".dagSearch-amount2").val(app.plugins.DAGsearchWui.formatDate(stopTo));						
							var cql = field.cql;
							cql = cql.replaceAll("{{field}}",field.name );
							cql = cql.replace("{{from}}",app.plugins.DAGsearchWui.formatDT(stopFrom) );
							cql = cql.replace("{{to}}",app.plugins.DAGsearchWui.formatDT(stopTo)  );
							control.attr("userFilter",cql);
							app.plugins.DAGsearchWui.applyFilter();
						},
					...
					control.find(".dagSearch-amount1").on('blur', function() { // update slider and prevent min > max
						...
						if (newMinVal < control.find(".dagSearch-slider-range").slider( "values", 1) ) {
							...
							var cql = field.cql;
							cql = cql.replaceAll("{{field}}",field.name );
							cql = cql.replace("{{from}}",app.plugins.DAGsearchWui.formatDT(new Date($(this).val())) );
							cql = cql.replace("{{to}}",app.plugins.DAGsearchWui.formatDT(new Date(control.find(".dagSearch-slider-range").slider( "values", 1))));
							control.attr("userFilter",cql);
							app.plugins.DAGsearchWui.applyFilter();	
						} else {
							$(this).val(app.plugins.DAGsearchWui.formatDate(new Date(control.find(".dagSearch-slider-range").slider( "values", 0))));
						}
					});
					control.find(".dagSearch-amount2").on('blur', function() { // update slider and prevent max < min
						...
						if (newMaxVal > control.find(".dagSearch-slider-range").slider( "values", 0) ) {
							...
							var cql = field.cql;
							cql = cql.replaceAll("{{field}}",field.name );
							cql = cql.replace("{{from}}",app.plugins.DAGsearchWui.formatDT(new Date(control.find(".dagSearch-slider-range").slider( "values", 0))));
							cql = cql.replace("{{to}}",app.plugins.DAGsearchWui.formatDT(new Date($(this).val()))  );
							control.attr("userFilter",cql);
							app.plugins.DAGsearchWui.applyFilter();					
							
						} else {
							$(this).val(app.plugins.DAGsearchWui.formatDate(new Date(control.find(".dagSearch-slider-range").slider( "values", 1))));
						}
					});
	...
	*/
	sendIds() {
		...
		var url = app.plugins.DAGsearchWui.identQueue[0].transmitter+"?sessionId="+sessionId;
		...
		app.plugins.DAGsearchWui.featureStore.forEach(function(feat,index) {
...
//			message = "Sending "+nameList+"... ";
		...
		.fail(function(jqxhr, settings, exception) {
			logger("ERROR", app.plugins.DAGsearchWui.name + ": Error sending search result to POSSE");
			return;
		}); 
	}	
	...
	*/
	toggleVector(active) {
		...
		if (app.plugins.DAGsearchWui.vectorDraw) app.map.removeInteraction(app.plugins.DAGsearchWui.vectorDraw);
		app.plugins.DAGsearchWui.markupSource.clear();
		...
		if (active) {
			...
			app.plugins.DAGsearchWui.vectorDraw.on("drawend", function (e) {
				e.feature.setStyle(style);
				// Reset the map to default
				app.map.removeInteraction(app.plugins.DAGsearchWui.vectorDraw);
				active = false;
	...
	findFeatures() {
		...
		if (app.plugins.DAGsearchWui.markupSource.getFeatures().length == 0) {
		...
		app.plugins.DAGsearchWui.featureStore = [];
		app.plugins.DAGsearchWui.resultsTreeData = [];
		...
		// Add the tab spinner
		var spinner = new Spinner(app.spinnerOptionsMedium).spin($(app.plugins.DAGsearchWui.tabContent)[0]);

		// Define misc variables
		var layersToSearchCount = app.plugins.DAGsearchWui.identQueue.length;
		...
		var callback = function(features){
			...
			if (features.length > 0) app.plugins.DAGsearchWui.addToResultsTreeData(features);
			...
			if (layerSearchCompletedCount >= layersToSearchCount) {
				// Show results
				app.plugins.DAGsearchWui.showResults();
		...
		var searchPolygonInLayerNativeProj = transformFeature(app.plugins.DAGsearchWui.markupSource.getFeatures()[0],app.map.getView().getProjection().getCode(),"EPSG:3005"); 
		var wktFormat = new ol.format.WKT();
		var searchPolygonWKT = wktFormat.writeGeometry(searchPolygonInLayerNativeProj.getGeometry());
		
		app.plugins.DAGsearchWui.identQueue.forEach(function(l,index){
			l.id = index;

			// Build the CQL_FILTER statement 
			var existingFilter = app.plugins.DAGsearchWui.getCqlFilter();
			...
			var options = {
				...
				count: app.plugins.DAGsearchWui.pcfg.proximitySearchMaxResults,
			...
			// Handle a failure
			.fail(function(jqxhr, settings, exception) {
				logger("ERROR", app.plugins.DAGsearchWui.name + ": Error querying features");
				callback([]);
				return;
			}); 
	...
	notifyUserOfFailure(errorText) {
			bootbox.dialog({
				title: "DAGsearchWui query error",
	...
	addToResultsTreeData(features) { 
		...
		$.each(features, function(index,feature) {			
			var checkbox = '<input type="checkbox" title="Checked locations will be sent" class="form-check-input dagSearchCheck" checked id=dagSearchCheck-feature-' + (app.plugins.DAGsearchWui.featureStore.length)+'>'; 
			...
			app.plugins.DAGsearchWui.featureStore.push(feature);
			
			// Build the child (feature) node
			var childNode = new Object();
			childNode.id = "dagSearch-feature-" + (app.plugins.DAGsearchWui.featureStore.length - 1);
		...
		app.plugins.DAGsearchWui.resultsTreeData.push(parentNode);
	}
		...
		*/
		...
		if (app.plugins.DAGsearchWui.resultsTreeData.length > 0) {
			...
			$(".dagSearch-treeview-feature").click(function(node){
				...
				var feat = app.plugins.DAGsearchWui.featureStore[featureStoreId];
			...
			$(".dagSearch-treeview-feature").hover(
				function(){
					// Get the feature
					var featureStoreId = this.id.replace("dagSearch-feature-", "");
					var feature = app.plugins.DAGsearchWui.featureStore[featureStoreId];
	...
	addPlugin() {
		...
		var callback = function(success, tabNav, tabContent){
			// Bail if failed
			if (!success) {
				logger("ERROR", app.plugins.DAGsearchWui.name + ": Plugin failed to initialize");
				return;
			}
			
			// Set class variables
			app.plugins.DAGsearchWui.tabNav = tabNav;
			app.plugins.DAGsearchWui.tabContent = tabContent;
			...
			$(".dagSearch-get-location-from-map-btn").click(function(){ 
				...
				app.plugins.DAGsearchWui.toggleVector(true);
			});
			...
			$(".dagSearch-search-point .dagSearch-clear-input-btn").click(function(){ 
				...
				$("#dagSearch-results").empty();
				$("#dagSearch-no-results").hide();
				app.plugins.DAGsearchWui.toggleVector(false);
			});
			...
			$("#dagSearch-overlay-search-btn").click(function(){
				...
				app.plugins.DAGsearchWui.findFeatures();
			...
			$("#dagSearch-overlay-sendAll-btn").click(function(){
				...
				// Handle the response
				.done(function(response) {
					app.plugins.DAGsearchWui.sendIds();
				})
			...
			app.plugins.DAGsearchWui.addFilterControls();
			
			// Log success
			logger("INFO", app.plugins.DAGsearchWui.name + ": Plugin successfully loaded");
		}
