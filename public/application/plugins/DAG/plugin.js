class DAG {
	...
	constructor() {
		...
		this.tabContentFile = "application/plugins/DAG/tab-content.html";
		this.response = "";
	...
	search(request, callback, options) {
		...
		if (app.plugins.DAG.pcfg.geoCoderEnabled) {
			providersToSearchCount++;
			app.plugins.DAG.searchBcGeoCoder(request, overallResultsAggregator, options);
		}
		
		// Search visible (and configured) layers if configured
		if (app.plugins.DAG.pcfg.visibleLayerSearchEnabled) {
			providersToSearchCount++;
			app.plugins.DAG.searchLayers(request, overallResultsAggregator, options);
		}
	}
	...
	searchBcGeoCoder(request, callback, options) {
		...
		var params = {
			minScore: 50,
			maxResults: app.plugins.DAG.pcfg.geoCoderMaxResults,
			echo: 'false',
			brief: true,
			autoComplete: true,
			outputSRS: outputSRS,
			addressString: request.term,
			apikey: app.plugins.DAG.pcfg.geoCoderApiKey
		};
		...
		$.each(app.map.getLayers().getArray(), function(index, layer) {
			...
			if (layer.get("visible") && searchableFields.length > 0) {			
				...
				var params = {
					...
					typeNames: layers,
					outputFormat: "json",
					count: app.plugins.DAG.pcfg.visibleLayerSearchMaxResults,
	...
	registerSearchRadius() {
		...
		app.plugins.DAG.searchRadiusLayer.getSource().clear();
		...
		app.plugins.DAG.searchRadiusFeature = new ol.Feature(searchPolygon);
		
		// Show that search radius polygon (if configured for app)
		if (app.plugins.DAG.pcfg.showSearchRadiusOnMap) {
			app.plugins.DAG.searchRadiusLayer.getSource().addFeature(app.plugins.DAG.searchRadiusFeature);
		}
		
		// Zoom to that search radius feature
		zoomToFeature(app.plugins.DAG.searchRadiusFeature);
	...
	findFeatures() {
		...
		app.plugins.DAG.featureStore = [];
		app.plugins.DAG.resultsTreeData = [];
		...
		// Add the tab spinner
		var spinner = new Spinner(app.spinnerOptionsMedium).spin($(app.plugins.DAG.tabContent)[0]);

		// Define misc variables
		var layersToSearchCount = app.plugins.DAG.identQueue.length;
		var layerSearchCompletedCount = 0;
		...
		var callback = function(features){
			...
			if (features.length > 0) app.plugins.DAG.addToResultsTreeData(features);
		...
		var searchPolygonInLayerNativeProj = transformFeature(app.plugins.DAG.searchRadiusFeature, app.map.getView().getProjection().getCode(),"EPSG:3005"); 
		var wktFormat = new ol.format.WKT();
		var searchPolygonWKT = wktFormat.writeGeometry(searchPolygonInLayerNativeProj.getGeometry());
		
		app.plugins.DAG.identQueue.forEach(function(l,index){
			l.id = index;
			...
			var options = {
				...
				count: app.plugins.DAG.pcfg.proximitySearchMaxResults,
				srsName: app.map.getView().getProjection().getCode(),
				cql_filter: cqlFilter
			}
			...
			// Handle a failure
			.fail(function(jqxhr, settings, exception) {
				logger("ERROR", app.plugins.DAG.name + ": Error querying features");
				callback([]);
				return;
			}); 
	...
	addToResultsTreeData(features) { 
		...
		$.each(features, function(index,feature) {			
			...
			app.plugins.DAG.featureStore.push(feature);
			
			// Build the child (feature) node
			var childNode = new Object();
			childNode.id = "dag-feature-" + (app.plugins.DAG.featureStore.length - 1);
		...
		app.plugins.DAG.resultsTreeData.push(parentNode);
	}
	...
	showResults() {	
		...
		*/
		...
		if (app.plugins.DAG.resultsTreeData.length > 0) {
			// Add a tree view div
			$("#dag-results").append("<div id='dag-tree'></div>");
			
			// Register and build the treeview
			$("#dag-tree").bstreeview({
				data: app.plugins.DAG.resultsTreeData,
			...
			$(".dag-treeview-feature").click(function(){
				...
				var feat = app.plugins.DAG.featureStore[featureStoreId];
				...
				try {
					...
					logger("ERROR", app.plugins.DAG.name + ": "+transmitter+" not found");
				}
			...
			$(".dag-treeview-feature").hover(
				function(){
					// Get the feature
					var featureStoreId = this.id.replace("dag-feature-", "");
					var feature = app.plugins.DAG.featureStore[featureStoreId];
	...
	addPlugin() {
		...
		var callback = function(success, tabNav, tabContent){
			// Bail if failed
			if (!success) {
				logger("ERROR", app.plugins.DAG.name + ": Plugin failed to initialize");
				return;
			}
			
			// Set class variables
			app.plugins.DAG.tabNav = tabNav;
			app.plugins.DAG.tabContent = tabContent;
			...
			$(".dag-search-point .dag-get-location-from-map-btn").click(function(){ 
				...
				app.plugins.DAG.registerMapClickLocation(associatedInput);
			});
			
			// Register the category autocompletes
			$(".dag-search-point .dag-location-input").catcomplete({
				minLength: 3,
				source: app.plugins.DAG.search,
				autoFocus: true,
				select: function(event, selection) {
					app.plugins.DAG.registerSearchSelectionLocation($(this), selection.item);
				}
			});
			...
			$(".dag-search-point .dag-get-geolocation-btn").click(function(){ 
				var associatedInput = $(this).parent().find(".dag-location-input");
				app.plugins.DAG.registerDeviceLocation(associatedInput);
			});
			...
			$("#dag-search-distance-input").val(app.plugins.DAG.pcfg.defaultSearchRadiusM);
			
			// Enable/disable the search distance input as per the application configuration
			$("#dag-search-distance-input").prop("disabled", !app.plugins.DAG.pcfg.searchRadiusEditable);
			...
			$("#dag-overlay-search-btn").click(function(){
				app.plugins.DAG.findFeatures();
			});
			
			// Log success
			logger("INFO", app.plugins.DAG.name + ": Plugin successfully loaded");
		}
