...
// edit line 83 (baseUrl)	
	...
	constructor() {
		...
		modify.on("modifyend",function(evt) { 
			app.plugins.DAGaddPointWui.confirmPoint(app.plugins.DAGaddPointWui.PointsLayer.getSource().getFeatures()[0].getGeometry().getCoordinates());
		});	
		...
		/*
		* Function: openedWithCurrentLocation()
		* Returns true if map opened from a single location 
		* @param ()none
		...
		* Returns the currentLocation if there was one
		* @param ()none
		* @returns (string) BC Albers X,Y
		*/
		function getCurrentLocation() {
			var retVal = null;
			if (post) {
				if (post.currentLocation) {
					retVal = post.currentLocation;
				}
			}
			return retVal;
		}
		/*
		* Function: openedWithOtherLocations()
		* Returns true if map opened from a single location 
		* @param ()none
		* @returns (boolean) default is false
		*/
		...
		*/
		function getOtherLocations() {
			...
			if (post) {
				if (post.otherLocations) {
					retVal = post.otherLocations.replaceAll(";",",");
				}
			}
			return retVal;
		}
		...
		function hightlightAndZoomApplicant() {
			...
			if (openedWithCurrentLocation()) {
				var bCAlbers = [];
				var xYpair = getCurrentLocation().split(",");
				console.log("here now in CurentLocation");
			...
			if (openedWithOtherLocations()) {
				...
				for (var pairCount = 0; pairCount <= otherPairs.length/2 +2; pairCount += 2) {
					...
					var mapFeature = new ol.Feature(new ol.geom.Point(mapPoint));
					app.highlightLayer.getSource().addFeature(mapFeature);
					console.log("mapFeature coordinates: "+mapFeature.getGeometry().getCoordinates());
				}
			}
			if (!openedWithCurrentLocation()&&openedWithOtherLocations()) {
				app.map.getView().fit(app.highlightLayer.getSource().getExtent(), {padding: [20,20,20,20]});
			}
		...
		if (post.site.includes("applicant")) {
	...
	/*
	* Function: sendIt()
	* Returns per layer import feature function
	* @param (string)transmitter: name of maplink.aspx function
	* @param (string)value: name of road or admin area or list for first people consultations
	* @returns () none
	*/
	sendIt(transmitter,value) {
		var message = [transmitter,value];
		top.postMessage(message, "*");
		var _value = value.replaceAll(" ","_");
		// update UI to inform user
		$("#"+_value).css("color","red");
		$("#popup-content").html("Sending "+value+"... ");
	...
	* Displays an image in a modal box
	* @param (string) winTitle: text to display above the image 
	* @param (string)imageURL: location of image
	* @returns () none
	*/

	openImageWindow(winTitle, imageURL) {
		bootbox.dialog({
			title: winTitle,
			size: 'extra-large',
	...
	confirmPoint(coords) {
		...
		let thisPoint = app.plugins.DAGaddPointWui.PointsLayer.getSource().getFeatures()[0];
		let friendlyPoint = displayXY[1].toFixed(5) + "&deg;, " + displayXY[0].toFixed(5) + "&deg;"
		var message = "<br>Choose Yes to confirm. ";
		message += app.plugins.DAGaddPointWui.devTech? "<input type='checkbox' "+(app.plugins.DAGaddPointWui.launchImport?"checked":"")+" id='dag-add-pt-launchImport' value='Y'> Launch Import Features.":"";
				...
				confirm: {
					...
					callback: function() {
						...
						try {
							...
							logger("ERROR","DAGaddPointWui: Error communicating with Posse");
						}
						...
						if (app.plugins.DAGaddPointWui.devTech) {
							...
							if ( $("#dag-add-pt-launchImport")[0].checked )  {
								...
								app.plugins.DAGaddPointWui.launchImport = false;
							}
						}
						// Reset the map's single click function to default "move" point to added layer
						resetDefaultMapSingleClickFunction();
						app.plugins.DAGaddPointWui.AddedPointsLayer.getSource().addFeature(app.plugins.DAGaddPointWui.PointsLayer.getSource().getFeatures()[0]);
						app.plugins.DAGaddPointWui.PointsLayer.getSource().removeFeature(thisPoint);
	...
	mapClickFunction(e){
		...
		app.plugins.DAGaddPointWui.PointsLayer.getSource().addFeature(pointFeature);
		
        // Ask user if this is where they want it
        // app.plugins.DAGaddPointWui.PointsLayer.getSource().getFeatures()[0].getGeometry().getCoordinates()
		app.plugins.DAGaddPointWui.confirmPoint(e.coordinate);
	...
	addPlugin() {
		...
		var iconSrc = '<img src="./application/plugins/DAGaddPointWui/img/map-marker-8x.png" width="20" height="20">';
		...
		if (!(post.allowUpdate=="yes")) {
			...
			$("#dag-location-btn").click(function() {
				...
				redirectMapSingleClickFunction("crosshair", app.plugins.DAGaddPointWui.mapClickFunction);
			});
		}
		// Log success
