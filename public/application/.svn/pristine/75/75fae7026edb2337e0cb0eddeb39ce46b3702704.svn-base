...
//		$(conn).find(".mfn-device-name").html(feat.properties.DEVICE_NAME);
		...
		app.plugins.MFNaddConnection.connected.push(feat.properties.LINK_ID);
		// disconnecting network link NOT YET IMPLEMENTED
		...
		if (app.plugins.MFNaddConnection.canEdit) {
			...
			$(conn).find(".mfn-connection-nix").html(buttonText).click(function(){
				...
				if (isNew) {
					...
					console.log("about to cut here");
					app.plugins.MFNaddConnection.makeConnection(location_id, thisRow.find(".mfn-lhs-cable").data().strand, thisRow.find(".mfn-rhs-cable").data().strand2, 'X',this_distance ,thisRow,true,connection_id);	
			...
			if (isNew) { // allowed to connect these two		
				$(conn).find(".mfn-connection-ok").html('<button type="button" class="btn btn-outline-success btn-sm"><span class="oi oi-check"></span></button>').click(function(){
					...
					app.plugins.MFNaddConnection.makeConnection(location_id, thisRow.find(".mfn-lhs-cable").data().strand, thisRow.find(".mfn-rhs-cable").data().strand2, thisRow.find(".mfn-connection-type input[name='"+thisRow.find('.mfn-rhs-cable').data().strand2.LINK_ID+"']:checked").val(),this_distance ,thisRow,false,null);
					app.plugins.MFNaddConnection.manageConnectionsAt( location_id );
				});
			} else {
				// read only
	...
	*/
	makeConnection(location_id,lhs,rhs,cType,this_distance,fieldSetRow,isCut,connection_id) {
		...
		var is_join; // will be set to Oracle flag Y or N
		var async = false;
		...
		xmlns:ogc="http://www.opengis.net/ogc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			<wfs:Insert typeName="iss:ISS_MFN_CONNECTION_EDIT">
				<iss:ISS_MFN_CONNECTION_EDIT xmlns:gml="http://www.opengis.net/gml">
					...
					<iss:IS_JOIN>${is_join}</iss:IS_JOIN>
					<iss:RHS_LINK_ID>${rhs.LINK_ID}</iss:RHS_LINK_ID>
				</iss:ISS_MFN_CONNECTION_EDIT>
			</wfs:Insert>
		</wfs:Transaction>
		`;
...
console.log("DISPLAY CABLE AS LINE BETWEEN ENPOINTS, ALSO ADD CABLE_ID TO SORT CABLES WITH SAME NAME");
					// highlight cable on hover
					var cableGeom = new ol.geom.LineString([feat.properties.ST_GEO.coordinates[0],feat.properties.ST_GEO.coordinates[1],feat.properties.END_GEO.coordinates[0], feat.properties.END_GEO.coordinates[1]]);
				...
				if (app.plugins.MFNaddConnection.canEdit) {		
					...
					if ((feat.properties.ST_LOC_ID != location_id) && (feat.properties.END_LOC_ID != location_id)) {
						$(strand).find(".mfn-lhs-cut").data("strand",feat.properties).html('<button type="button" class="btn btn-outline-danger btn-sm"><span class="mfn-connection-cut-icon"></span></button>').click(function(){ // after 
							app.plugins.MFNaddConnection.makeConnection(location_id, feat.properties, feat.properties, "X",this_distance ,$(strand),true);	  
							app.plugins.MFNaddConnection.manageConnectionsAt(location_id);
						} );
					}
				}						
			...
			.done(function(conduitPoint) {
				if (conduitPoint.features) {
					...
					.done(function(conduit) {
						...
						console.log("Conduit count = "+app.plugins.MFNaddConnection.thisLocation.conduits.length);
